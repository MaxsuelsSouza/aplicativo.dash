...
interface PropriedadesDoCalendarioComDeslize {
  registros: Registro[];
}

export default function CalendarioComDeslize({ registros }: PropriedadesDoCalendarioComDeslize) {
  ...
  const rolarParaMesAtual = useCallback(() => {
    if (referenciaDoScroll.current) {
      const deslocamento = mesAtual * larguraDaTela;
      referenciaDoScroll.current.scrollTo({ x: deslocamento, animated: true });
    }
  }, [mesAtual, larguraDaTela]);
  ...
  const renderizarMes = (indiceMes: number) => {
    ...
    return (
      <View key={indiceMes} style={[styles.containerMes, { width: larguraDaTela }]}>
          ...
          {semanas.map((semana, indiceSemana) => (
            <View key={indiceSemana} style={styles.linhaSemana}>
              {semana.map((dia, indiceDia) => {
                ...
                const ehFimDeSemana = diaDaSemana === 0 || diaDaSemana === 6; // Domingo (0) ou Sábado (6)
